...
It derives its super powers from it.
...
Templates are instances of Schema that provide pre-defined values or default
values and may add additional constraints tailored to a specific use case.
Policy are instances of Schema that define additional constraints
...
A configuration `B` is said to be backwards compatible with `A` if for all
