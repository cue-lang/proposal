...
Initially, we will use a model where an agent (a GitHub App; see [related
proposal](TODO)) will copy content to the central registry on behalf of
...
by name (a slash-separated path). A repository holds a set of _tags_, also
addressed by name (alphanumeric), each of which refers to a _manifest_,
...
`[github.com/foo/bar@v1.2.3](http://github.com/foo/bar)` will be stored as
...
central registry will have a fully populated set of repositories and
