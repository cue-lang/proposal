...
```go
func PureFunc1[A0, R any](f func(A0) (R, error), opts ...FuncOption) Value
func PureFunc2[A0, A1, R any](f func(A0, A1) (R, error), opts ...FuncOption) Value
// ... through PureFunc10
```

Each `PureFuncN` wraps a Go function of N arguments into a CUE `Value`
...
into a definition (`#add`). This is the typical pattern: define a
placeholder (`#add: _`) in CUE code, then fill it from Go.
...
Both `PureFuncN` and `ValidatorFunc` accept optional `FuncOption` values.
...
- **Variable-arity functions.** Each `PureFuncN` requires exactly N
  arguments. There is no support for optional arguments or variadic
  calls.

- **Non-concrete arguments.** All arguments must be concrete (fully
  resolved) CUE values. Passing a constraint or incomplete value as an
...
### Why PureFunc and not just Func?
...
The `PureFuncN` naming pattern reserves a range of names in the `cue`
...
- `cue/func_gen.go`: Generated `PureFunc1` through `PureFunc10`
...
A proof-of-concept implementation exists. See also the companion
proposals on [value injection](./4294-value-injection.md) (which
provides a mechanism for injecting functions into CUE packages without
`FillPath`) and [tagged string
literals](./language/4295-tagged-string-literals.md) (which demonstrates
a use case for user-provided functions).
