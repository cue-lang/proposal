...
We also propose introducing the `self` keyword to refer to the current scope (the value to the right of the innermost colon `:`). `self` can be used to refer to the top of the file, which was not possible before. The expressions `x: op(self)` and `x~Self: op(Self)` are equivalent.
...
Referring to the top of the file This was not possible before. Use self to refer to the top of the file.
```
let Top = self
