...
The _location_ of modules will be determined by a _registry configuration_.
...
- the first layer (layer 0) represents the actual content of the module's archive. The media type
of this layer is `application/zip`.
...
Note: currently the above CUE media types are not yet IANA-registered.
We plan to register them in future.
...
known set of network addresss (as [specified above](#customizing-name-resolution)
by `$CUE_REGISTRY`) to fetch all dependencies. This
...
- Copying modules from the central registry into a custom registry.
- Uploading private modules to a custom registry only.
...
- assuming immutability of module versions, local disk-based caching
can be used to avoid fetching a given version of a module more than once.
