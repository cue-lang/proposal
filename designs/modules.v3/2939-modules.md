...
To address this demand, we propose that CUE modules will be pulled
from [OCI](https://opencontainers.org/) [registries](https://github.com/opencontainers/distribution-spec/blob/main/spec.md),
and that there will be a public registry
available to serve as a central repository for CUE _modules_. A module
is a collection of packages that are released, versioned, and
distributed together. The central registry will allow users to share
and discover CUE modules and will provide versioning and dependency
management features.
...
to determine whether some user might have permission to upload
or access a module, but in general the existence of a module at a
given path does not imply that the module can be fetched from that path.

A [registry configuration](./2941-modules-storage-model#registry-configuration)
defines the mapping from an abstract module path
to the actual location where that module is stored.
