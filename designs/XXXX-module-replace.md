...
Aliasing one module path to another is not, however, semantically free
in CUE: it interacts with CUE's per-package hidden-field namespaces
when both the replaced module and its target are used in the same
configuration. Whether — and how — such aliasing should be permitted is
discussed in "Open questions: aliasing and hidden fields" below.
...
Module-version replacement that redirects one module path to a
differently-named module (see "Module-version replacement" and
"Replace field syntax") raises an unresolved question about how the two
module paths relate to each other. It is recorded here for the design
discussion.
...
the same configuration. The reasoning is that allowing both leaves the
two packages distinct, whereas making them interoperate through the
replace amounts to treating them as the *same* package, and CUE's
per-package hidden-field namespaces make the difference between
"distinct" and "the same" observable.
...
`foo` and `bar` as aliases means treating values drawn from the two
packages as interchangeable. But two packages that are *already* both
imported cannot always be safely equated after the fact: if a
...
Against disallowing aliasing, there is a concrete and common use case
that argues for supporting it: a widely-used module whose canonical
import path has migrated to a new one. Much of the ecosystem adopts the
new path, but some dependencies still import the old one. Redirecting
the old path to the new one is precisely what a module-version replace
is meant to handle, and for it to be useful the parts of the dependency
graph that have already switched to the new package must continue to
interoperate with the parts still using the old one through the
replace — which is exactly aliasing the two together. It would seem
reasonable, too, for the old module to become a thin forwarding module
to the new one without that counting as a breaking change, and that is
essentially the same operation. On this view the aliasing use case is
both more useful and more likely in practice than a configuration that
imports the replaced module and its target simultaneously and depends
on their hidden-field namespaces staying distinct.
...
namespace — which defeats the aliasing use case. If they are not kept
distinct, the collision problem above can arise. Resolving this
tension — including deciding whether the initial implementation should
simply disallow aliasing (rejecting a replacement whose target module
path differs from the dependency it replaces, or whose target is also
imported directly) until the semantics are settled — is left to the
design discussion.
