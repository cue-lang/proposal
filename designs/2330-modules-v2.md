...
interface with this web service through the `cue` command line or a GitHub app.
...
Most package managers for programming languages or schema stores follow a
registry design. For example Rust with [crates.io](https://crates.io/) and
JavaScript with [NPM](https://www.npmjs.com/).

Under such a design, a module _author_ _publishes_ a version of a module to a
registry.  Module _consumers_ or _users_ then _resolve_ and _download_
dependencies via the registry.

Go deviates from this approach by using a [proxy](https://proxy.golang.org/).
...
- Performance:
    - Cloning repositories can be costly. Using a VCS-backed proxy may result in
      longer download times when dealing with empty caches or when a proxy is
      not available. Since CUE is often used in such contexts, this concern is
      even greater.
- Enhanced security:
    - Considerably smaller attack surface.
...
- Possibility for enhanced functionality:
    - Features like cross-validation become considerably easier to implement.

The historical context that led to the choice of the proxy design for Go does
not apply to CUE. It seems likely that Go would have opted for a registry design
if the historical context would have permitted it.
...
will come later and is discussed in a separate document.

The registry will supply cryptographic content hashes for modules and modules
themselves will contain hashes of their dependencies. This will allow users to
check module contents even in the presence of untrusted registries.
...
Semver v2 adds some constraints on v1 that seem reasonable to adopt:

- Does not allow leading zeros.
- Minor version must be updated if feature is deprecated.
...
between minor and patch versions can be useful for schema considerations.
...
automated validation during upload and prevents accidents. We plan to provide
convenience functionality in the `cue` CLI  for bumping the major version.
...
- Many package managers use a `$user/$pkgname` or `$pkgname` approach to
  ...
  phenomenon and continues to be a common abuse pattern.
...
- There is a single way of marking a major version and a single module path to
  identify all major versions.
    - For instance, for Go modules, `example.com/foo/v1` and
      ...
      same module. In CUE, these would always be different modules.  For the
      interpretation of two different major versions of the same module, the
      corresponding CUE identifiers would look like `example.com/foo@v1` and
      `example.com/foo@v2`.
- Go special cases `/vx` directories to allow different major modules to share
  the same path in a VCS repo. The resulting ambiguities introduce a great deal
  of complexity and confusion. The proposed approach avoids avoids this.
- The approach is flexible enough to allow different representations on an
  underlying VCS, including
    - always use the same directory
    - use the same directory on different branches
    - use different paths in the VCS altogether to allow multiple major versions
      to coexist
- This approach allows future support for more precise version imports or even
  importing multiple non-major versions at once.
    - This can be useful when computing schema that represent a fleet of server
      API versions.
    - Some users have expressed interest in having more accurate pinning than
      major version.

In all the above cases, the `cue.mod/module.cue` would use the same module path
with different major version suffixes. This is possible because the CUE registry
stands by itself and is agnostic of where a module resides on VCS
...
`example.com/foo/bar`. Down the line we will probably allow this to allow for
module “splitting”.
...
As a consequence, though, the `cue.mod/module.cue` file needs to be able to
indicate a default version in case the CUE code depends on two major versions of
a given module.
...
Compatibility guarantees only go so far. Module consumer should remain cautious
...
of a module to avoid breakage. We envision having usage guidelines as well as
tooling support to avoid such pitfalls.
...
- allow module publishers to opt in to stricter rules;
- provide “virtual” computed schema that reflect best practices for a range of
  versions;
...
fields](https://github.com/cue-lang/cue/discussions/1951). Required fields are
...
[`go.sum`](https://go.dev/ref/mod#go-sum-files) file.
...
```
cue mod tidy [-i] [-v] [-go=version] [-compat=version]
```
...
attributes, even if those source files wouldn't normally be built. There one
...
The `cue mod publish` command uploads the [main
module](#main-module) to
the registry and tags it with the given version. If no version is given, it
selects the next available minor or patch release.
...
The `cue mod upload` command uploads the [main
module](#main-module) to
the registry and reports a unique identifier for this module.
...
public packages cannot be retracted.
...
The `-x` flag causes `download` to print the commands `download` executes to
standard error.
...
- A GitHub user can install the Github app to their personal account, or an
  organization they control, to automatically upload certain repos to the CUE
  registry.
- Uploads are triggered by tagging a release as `cue-v$semver`, where the
  `$semver` part must be a valid SemVer tag. e.g. `cue-v1.2.3`.
- A repository may have multiple CUE modules, each indicated by a `cue.mod`
  subdirectory.
- The module path of each module must be of the form
  `github.com/$user/repo/path@$majorversion` or
  `github.com/$org/repo/path@$majorversion` and correspond to
  the location in the repository.
- For a given repository, only tags with the same major version are uploaded.
- Otherwise, a module is only uploaded if:
...
- The main aim of the design is to have something out quickly.
...
We plan to use [Minimum Version
Selection](https://go.dev/ref/mod#minimal-version-selection) for resolving
dependency versions.
...
- Compared to a SAT solver: MVS has much more predictable running times.
- We considered using an “always on HEAD” approach.
...
* Phase 1
  * Initial version of the registry implemented
  * Publishing to registry implemented via GitHub app
  * `cue mod tidy` implemented
* Phase 2
  * GitHub auth-based registry accounts for publishers
  * `cue mod publish` implemented
  * GitHub app retained for those people who want Go-style "auto publish"
