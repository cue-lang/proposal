...
with supporting co-versioning other languages in the future.
...
- Performance:
    - Cloning repositories can be costly. Using a VCS-backed proxy may result in
      ...
      even greater.
- Enhanced security:
    - Considerably smaller attack surface, because VCS interaction is taken out
      of the equation.
...
- Minor version must be updated if a feature is deprecated.
...
- There is a single way of marking a major version and a single module path to
  identify all major versions.
    - For instance, for Go modules, `example.com/foo/v1` and
      ...
      CUE, these would always be different modules.  For the interpretation of
      two different major versions of the same module, the corresponding CUE
...
- Go special cases `/vx` directories to allow different major modules to share
  ...
  of complexity and confusion. The proposed approach avoids this.
...
- This approach allows future support for more precise version imports or even
  importing multiple non-major versions at once.
    - This can be useful when computing schemas that represent a fleet of server
      API versions.
...
a minor vs patch release are closely related to the concept of [backwards
compatibility](#backwards-compatibility) and will be more precisely defined when
`cue mod publish` is implemented (see [Implementation
...
When that  `vendor`  directory exists, the `cue` command will load packages from
